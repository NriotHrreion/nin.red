    ...
    update() {
        ...
        if(this.game.isDrawing) {
            ...
            if(block instanceof BlockAir && this.game.currentBlock.indexOf("oak_door") != -1) {
                this.map[this.getRealPosition(this.game.mousePosition.y)][this.getRealPosition(this.game.mousePosition.x)] = new window[this.game.currentBlock]("bottom");
            } else if(block instanceof BlockAir || this.game.currentBlock == "air") {
                this.map[this.getRealPosition(this.game.mousePosition.y)][this.getRealPosition(this.game.mousePosition.x)] = new window[this.game.currentBlock](this);
            }
            if(block instanceof BlockOakDoor && this.game.currentBlock == "air") {
                switch(block.part) {
                    case "top":
                        this.map[this.getRealPosition(this.game.mousePosition.y) + 1][this.getRealPosition(this.game.mousePosition.x)] = new window[this.game.currentBlock](this);
                        break;
                    case "bottom":
                        this.map[this.getRealPosition(this.game.mousePosition.y) - 1][this.getRealPosition(this.game.mousePosition.x)] = new window[this.game.currentBlock](this);
                        break;
        ...
        for(let y = 0; y < this.map.length; y++) {
            for(let x = 0; x < this.map[y].length; x++) {
                try {
                    if(this.map[y + 1][x] instanceof BlockAir && this.map[y][x] instanceof BlockSand) { // gravity is an illusion
